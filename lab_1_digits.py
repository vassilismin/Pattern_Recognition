...
def digit_mean(X, y, digit):
    ...
    for i in range(16):
        for j in range(16):
            mean.append(mean_value_at_pixel_of_digit(train_features, train_labels, digit,  pixel=(i, j)))
...
# print(digit_variance(train_features, train_labels, 9)[252])




def var_at_pixel(X, y, digit, pixel=(10,10)):
    index = [ind for ind,x in np.ndenumerate(y) if y[ind] == digit]
    val = [X[ind, :].reshape(16,16)[pixel] for ind in index]
    return np.var(val)
# print(var_at_pixel(train_features, train_labels, 0))
...
# print(vars(train_features, train_labels, 9))
...
def plot_clf(clf, X, y):
    ...
    clf.fit(X, y, principal_component_analysis=True)
    ...
    for i in range(10):
        ax.scatter(X0[y == i], X1[y == i], label=i, s=50, alpha=0.8)
...
class CustomNBClassifier(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        ...
        if self.use_unit_variance:
            ...
            # self.X_var = np.array([vars(train_features, train_labels, i) for i in range(10)])
        return self
    ...
    def gaussian_prob(self, digit, x):
        ...
        # print(log_prob_x_given_digit[0][0])
        return log_prob_x_given_digit[0][0]
