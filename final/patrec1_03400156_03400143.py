...
class CustomNBClassifier(BaseEstimator, ClassifierMixin):
    ...
    def gaussian_prob(self, digit, x):
        ...
        """Returns a float (logarithm of a conditional probability)"""
        ...
        if self.use_unit_variance:
            ...
            smooth = max(self.X_var[digit]) * 10 ** - 4
            prob_constant = (- 256 / 2) * np.log(2 * np.pi) - 0.5 * np.log((np.prod(self.X_var[digit]) + 10 ** - 9))
            log_prob_x_given_digit = prob_constant - 0.5 * np.sum([((x[0][i] - mean[0][i]) ** 2) / (self.X_var[digit][i] + smooth) ** 2 for i in range(256)])
            return log_prob_x_given_digit
...
# g.fit(train_features, train_labels)
# g.gaussian_prob(1, train_features[0].reshape(1, 256))
# print(g.score(test_features, test_labels))
